...
def その日に出せるか(もの, 日付, 定休日):
    ...
    if "土日" in 決まり:
        return 番号 >= 5
    えらんだ = [c for c in 曜日の字 if c in 決まり]
    return 字 in えらんだ if えらんだ else True
...
def えらぶ(並び, 履歴, 店舗, 日付, 枚数, かならず区分="", かならず枚数=1, なかをあける=2,
        時刻たち=None, 節=0, 定休日=""):
    """その日に出す画像をえらぶ。

    ・イベントの日は、その日の画像を先に
    ・商品リストの「出す曜日」「出す時刻」に合うものだけ
    ・同じ「なかま」（ハンバーグ／アサイーボウルなど）は何日かあける
    ...
    # 1回目：最近出したものと、同じ日に同じ商品・同じなかまが重なるのを避ける
    for もの in 候補:
        if len(選ぶ) >= 枚数:
            break
        鍵 = f"{店舗['id']}/{もの['ファイル']}"
        if もの["商品名"] in 出した商品 or なかま名(もの) in 出したなかま:
            continue
