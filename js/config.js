 ...
 * 原価のマス（現金仕入 F ＋ 掛仕入 G）
 *
 *   sumRow   … 累計の行。まとめの「税込累計」の行です
 *   moreRows … その累計の式から外れている、仕入先の行
 *
 * ★まとめの累計は `SUM(F4:F18)` で、**19〜23行の仕入先を足していません**
 *   （日別ページの当日総合計は `SUM(F4:F23)` で、そちらは足しています）。
 *   いまは19〜23行に数字が入りませんが、取引業者が増えれば使われます。
 *   落とすと原価がその分だけ少なくなるので、こちらから足して読みます
 *   （ko-dai の指示・2026年9月5日）。空のマスは0で返るので、読んでも害はありません。
 *
 * ★日報側の式が `SUM(F4:F23)` に直ったら、moreRows を空にしてください。
 *   直さないと、19〜23行を二重に数えます。
 */
function nippouCostCells(sumRow, moreRows) {
  const out = [`F${sumRow}`, `G${sumRow}`];
  (moreRows || []).forEach((r) => out.push(`F${r}`, `G${r}`));
  return out;
}
...
const NIPPOU_LAYOUTS_DEFAULT = [
  {
    ...
    cells: {
      ...
      ex: 'B25',       // 売上の税抜累計
      guests: 'B28',   // 客数累計
      // 原価＝24行の累計＋19〜23行（累計の式から外れている仕入先）
      cost: nippouCostCells(24, [19, 20, 21, 22, 23]),
      labor: 'G32',    // 人件費の当月累計
    },
  },
];
